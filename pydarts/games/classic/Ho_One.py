    ...
    def Sugestions_cible(self,players,actual_player,player_launch):
        ...
        if players[actual_player].points == 0 and (self.double_in or self.master_in):
            if not players[actual_player].unlocked:
                if self.double_in:
                    leds.extend(f'D{i}#{self.colors[8]}' for i in range(1, 21))

                if self.master_in:
                    leds.extend(f'T{i}#{self.colors[8]}' for i in range(1, 21))
                    leds.extend(f'D{i}#{self.colors[8]}' for i in range(1, 21))
                    leds.extend(f'S{l}#{self.colors[8]}' for l in ['B'])
                    leds.extend(f'D{l}#{self.colors[8]}' for l in ['B'])
        else :
            # Possible zap ?
            if self.zap:
                for p in players:
            ...
            leds.extend(f'{key}#{self.colors[8]}' for key in self.possible_hits)
