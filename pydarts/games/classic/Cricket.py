...
class Game(cgame.Game):
    ...
    def __init__(self, display, game, nb_players, options, config, logs, rpi, dmd, video_player):
        ...
        if self.display.qrcode is not None:
            self.display.qrcode.set_color((255,55,55),(200,200,200))
            self.display.qrcode.set_transparent(True)
            self.qrcode_icon = self.display.draw_qrcode('Cricket', bordure = 2)
...
# Game qrcode
        if self.qrcode_icon is not None:
            self.qr_x = int(self.display.res['x']) - (self.qrcode_icon.get_width() + 10)
            self.qr_y = option_y - self.qrcode_icon.get_height() - option_height
